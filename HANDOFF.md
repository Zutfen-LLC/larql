...
- `f32_gemv` / `f16_gemv` / most remaining `q4k_*` / `q6k_*` device kernels ~~— `f32_gemv`/`f16_gemv` landed in Session 8;~~ CUDA now has `q4k_matvec`, `q6k_matvec`, `q4k_matmul`, `q6k_matmul`, `q4k_dual_matvec`, `f32_gemv`, `f16_gemv`, `q4_matvec`, `q4_vecmat`, `kv_append`, `rms_norm`, `rms_norm_heads`, `geglu_silu`, `geglu_gelu_tanh`, `activation_silu`, `activation_gelu_tanh`, `residual_add`, `rope`, and `decode_attention`; the full `QuantMatVec` kernel surface is native, the KV-append primitive is native, all four elementwise families (body + per-head RMSNorm in Session 13, FFN activation in Session 14, residual add in Session 15, RoPE in Session 16) are native, and the decode-step attention itself is native (Session 17) (the device-kernel-fusion follow-on)
